	...
	<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" id="new_account_form" class="std" autocomplete="on" autofill="on">
		<fieldset>
			<div id="opc_account_choice" class="row">
				<div class="col-xs-12">
					<div class="opc-button">
						<span>
							<button type="submit" class="btn opc-button-medium" id="opc_guestCheckout"><span>{l s='Guest checkout'}</span></button>
						</span>
						<span>
							<button type="submit" class="btn opc-button-default" id="opc_createAccount"><span>{l s='Create an account'}</span></button>
						</span>
					</div>
				</div>
			</div>
			<div id="opc_account_form" class="unvisible">
				...
				{/if}
				<h3 class="page-subheading top-indent">{l s='Residential address'}</h3>
				{$stateExist = false}
				{$postCodeExist = false}
				{$dniExist = false}
